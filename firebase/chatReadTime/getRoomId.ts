 ...
 */
export const compareChatReadCounts = async (myStudentNumber: string, otherStudentNumber: string) => {
    try {
      // Step 1: 両者のユーザードキュメントIDを取得
      const usersRef = collection(db, 'users');
  
      const myQuery = query(usersRef, where('studentId', '==', myStudentNumber));
      const otherQuery = query(usersRef, where('studentId', '==', otherStudentNumber));
  
      const [mySnapshot, otherSnapshot] = await Promise.all([getDocs(myQuery), getDocs(otherQuery)]);
